...
Arrow is small and fast. With the innovative "[Application Loader & single task Kernel](/path/to/Arrow_Application_Loader_Single_Task_Kernel)", "[Kernel Sharing](/path/to/Arrow_Kernel_Sharing)", "[In-memory Filesystem](/path/to/Arrow_In_memory_Filesystem)" technologies and making use of non-simulation Hypervisor, Arrow can greatly decreases the overhead of running every single task on individual kernel and hypervisor. The performance is approaching that of container technology.
